...
import {DEFAULT_HTTP_USER_AGENT} from "../http/http-adapter.mjs";
...
export const DEFAULT_ROBOTS_PRODUCT_TOKEN = "SearchGrowthOS";

const MAX_ROBOTS_PRODUCT_TOKEN_BYTES = 512;
const ROBOTS_PRODUCT_TOKEN_PATTERN = /^[A-Za-z_-]+$/u;
...
export const DEFAULT_AUDIT_CONFIGURATION = Object.freeze({
  ...
  robots_product_token: DEFAULT_ROBOTS_PRODUCT_TOKEN,
});
...
function normalizedAuditConfiguration(configuration) {
  const config = { ...DEFAULT_AUDIT_CONFIGURATION, ...configuration };
  const productToken = config.robots_product_token;
  if (
    typeof productToken !== "string" ||
    !ROBOTS_PRODUCT_TOKEN_PATTERN.test(productToken) ||
    Buffer.byteLength(productToken) > MAX_ROBOTS_PRODUCT_TOKEN_BYTES
  ) {
    throw new TypeError(
      "robots_product_token must be 1-512 ASCII letters, underscores, or hyphens",
    );
  }
  return config;
}
...
export function createAuditRunner({
  ...
  return async function runAudit(kind, target, configuration = {}) {
    ...
    const config = normalizedAuditConfiguration(configuration);
